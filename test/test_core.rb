...
class TestSequentialProcess < Minitest::Test
  def test_start_sequential_process
    $stdout.sync = true
    test_str = 'hejsan'
    sp = SubProcess.new(SubProcess::NATIVE)
    info = sp.exec_sync('echo', test_str).task_info
    assert_equal("echo #{test_str}", info[:cmd_str])
    assert_equal(0, info[:process_status].exitstatus)
  ...
  def test_stdout_callback
    ...
    SubProcess.new(
      SubProcess::NATIVE,
      ->(line) { matches += 1 if line.start_with?('Reply from 127.0.0.1') }
    ).exec_sync('ping', ['-n', nof_pings, '127.0.0.1'])
  ...
  def test_completion_status
    $stdout.sync = true
    # expect this to complete with exit code 0
    sp = SubProcess.new(SubProcess::NATIVE).exec_sync('ping', ['-n', '1', '127.0.0.1'])
    assert_equal(sp.exit_zero?, true)

    # expect this to not have completed when the assert is executed
    sp = SubProcess.new(SubProcess::NATIVE).exec_async('ping', ['-n', '1', '127.0.0.1'])
    assert_equal(sp.exit_zero?, false)
    sp.wait_on_completion

    # expect this to complete with exit code != 0 since host does not
    # exist
    sp = SubProcess.new(SubProcess::NATIVE).exec_sync('ping', ['-n', '1', 'fake_host'])
    assert_equal(sp.exit_zero?, false)

    # expect this to never start a process since cmd not exists
    sp = SubProcess.new(SubProcess::NATIVE).exec_sync('pingggg', ['-n', '1', 'fake_host'])
    assert_equal(false, sp.exit_zero?)
    assert_equal(SubProcess::ExecutionState::FAILED_TO_START, sp.execution_state)
  end
  ...
  def test_start_two_parallel_processes
    ...
    p_array = msg_array.collect do |str|
      SubProcess.new(SubProcess::NATIVE).exec_async('echo', str)
    end
    SubProcess.wait_on_all(p_array)
  ...
  def test_block_yield_wait_all
    ...
    p_array = (1..4).collect do
      p = SubProcess.new(SubProcess::NATIVE)
      p.exec_async('ping', ['127.0.0.1'])
    end

    nof_finished = 0
    # wait on each process
    SubProcess.wait_on_all(p_array) do |p|
      nof_finished += 1
      info = p.task_info
      case p.execution_state
      when SubProcess::ExecutionState::ABORTED
    ...
    p_array.each do |p|
      assert_equal(p.execution_state, SubProcess::ExecutionState::COMPLETED)
    end
  end
  ...
  def test_back_to_back
    ...
    p_total = SubProcess.wait_or_back_to_back(p_array) do |p|
      nof_finished += 1
      raise 'Aouch' if p.execution_state == SubProcess::ExecutionState::ABORTED

      # create new processes as long as there are messages left
      unless messages.empty?
        np = SubProcess.new.exec_async(
          'echo', messages.shift
        )
      end
      np
